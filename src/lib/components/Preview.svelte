...
<div class="w-full h-full flex flex-col bg-base-200">
    <div class="flex justify-between items-center px-4 py-2 bg-base-200 border-b border-base-300 font-medium text-sm">
        <span>{viewMode === 'xml' ? 'TEI-XML Preview' : 'Rendered Text'}</span>
    ...
    <div class="flex-1 overflow-auto">
        {#if viewMode === 'xml'}
            <pre class="m-0 p-4 font-mono text-sm leading-relaxed whitespace-pre-wrap break-words"><code>{content}</code></pre>
        {:else}
